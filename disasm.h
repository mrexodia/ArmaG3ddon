...
unique int       farcalls=1;             // Accept far calls, returns & addresses
unique int       decodevxd=0;            // Decode VxD calls (Win95/98)
unique int       privileged=1;           // Accept privileged commands
unique int       iocommand=1;            // Accept I/O commands
unique int       badshift=1;             // Accept shift out of range 1..31
unique int       extraprefix=0;          // Accept superfluous prefixes
unique int       lockedbus=1;            // Accept LOCK prefixes
unique int       stackalign=1;           // Accept unaligned stack operations
unique int       iswindowsnt=1;          // When checking for dangers, assume NT
/*
int    __declspec(dllexport) __stdcall Assemble(char *cmd,ulong ip,t_asmmodel *model,int attempt,
         int constsize,char *errtext);
int    __declspec(dllexport) __stdcall Checkcondition(int code,ulong flags);
int    __declspec(dllexport) __stdcall Decodeaddress(ulong addr,char *symb,int nsymb,char *comment);
ulong  __declspec(dllexport) __stdcall Disasm(char *src,ulong srcsize,ulong srcip,
         t_disasm *disasm,int disasmmode);
ulong  __declspec(dllexport) __stdcall Disassembleback(char *block,ulong base,ulong size,ulong ip,int n);
ulong  __declspec(dllexport) __stdcall Disassembleforward(char *block,ulong base,ulong size,ulong ip,int n);
int    __declspec(dllexport) __stdcall Isfilling(ulong addr,char *data,ulong size,ulong align);
int    __declspec(dllexport) __stdcall Print3dnow(char *s,char *f);
int    __declspec(dllexport) __stdcall Printfloat10(char *s,long double ext);
int    __declspec(dllexport) __stdcall Printfloat4(char *s,float f);
int    __declspec(dllexport) __stdcall Printfloat8(char *s,double d);
*/
